const mainJs = (
  function(){
    'strict'
    const navBtn = document.querySelector('.top-header-left .nav-btn')
    const mainNav = document.querySelector('.top-header-left .main-nav')
    const mainNavCloseBtn = document.querySelector('.main-nav-close-btn')
    const mainNavContentContainer = document.querySelector('.main-nav-content-container')
    const btnCart = document.querySelector('.btn-cart')
    const cartSection = document.querySelector('.cart-section')
    const itemsCounter = document.querySelector('.btn-cart .items-quantity')
    const cartForm = document.querySelector('.cart-form')
    const inputProductQuantity = document.getElementById('product__quantity') 
    const cartProducts = document.querySelector('.cart-section-products')
    const emptyMsg = document.querySelector('.empty-msg')
    const cartBody = document.querySelector('.cart-section-body')
    const originalProductSlider = document.querySelector('.product-slider')
    const lightbox = document.querySelector('.lightbox')

    const modalLightbox = null
    let productIndex = 0
    ...
    function toggleBtnCheckout() {
      const btnCheckout = document.querySelector('.btn-checkout');
      const cartSize = cart.getCartSize();
    ...
    function manageProductClicks(event){
      ...
      if(element.matches(`.image-box__src[tabindex='0']`) && window.matchMedia(`(min-width: 992px)`).matches && actionCondition){
        ...
        if(element.matches(`[data-thumb-index], .thumb-item__btn`)){
          const localProductSlider = element.closest(`.product-slider`)
        ...
        else if(element.matches(`.icon-close, .btn-close-lightbox`)){
          setTimeout(() => {
    ...
    function slideProductImage(operator, productId, image){
      ...
      const localProductSlider = image.closest('.product-slider')
